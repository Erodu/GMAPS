...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        if (IsCaptain)
        {
            ...
            Vector3 directionVector = OtherPlayer.transform.position - startPosition;
            ...
            HVector2D hVecStartPos = new HVector2D(startPosition);
            HVector2D hVecDirection = new HVector2D(directionVector);

            float angle = hVecStartPos.FindAngle(hVecDirection);
